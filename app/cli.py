...
def _build_gemini_audit_config() -> GeminiAuditConfig:
    ...
    if not api_key:
        raise typer.BadParameter("缺少环境变量 GEMINI_API_KEY，无法执行 Gemini 自动审查。")
...
def _build_scene_anchor_prompt(*, character_name: str, scene_name: str, storyboard_text: str) -> str:
...
def _build_manju_video_prompt(*, character_name: str, scene_name: str, storyboard_text: str) -> str:
...
def _estimate_manju_duration_seconds(storyboard_text: str) -> int:
    """根据分镜摘要估算 Manju 更稳的短时长。"""

    text = storyboard_text.strip()
    score = 0

    if any(token in text for token in ("近景", "特写", "表情", "眼神", "脸", "嘴角", "咬牙")):
        score += 2
    if any(token in text for token in ("两人", "双人", "对峙", "看向", "指着", "指向")):
        score += 1
    if any(token in text for token in ("众人", "多人", "几个", "跟班", "同学们", "群像")):
        score += 2
    if any(token in text for token in ("挥拳", "收回拳头", "抬手", "指向", "转身", "起身", "走出去", "拉着", "搂着", "大笑")):
        score += 2
    if any(token in text for token in ("暴怒", "张狂", "恶狠狠", "疑惑", "激动", "急促")):
        score += 1
    if any(token in text for token in ("教室", "走廊", "操场", "报名处", "时光屋", "空间", "人群")):
        score += 1
    if any(token in text for token in ("进入", "离开", "走进", "走出", "进门", "出门")):
        score += 2
